...
LOG_FILE="/tmp/phase0-alignment.log"
PID_FILE="/tmp/phase0-alignment.pid"
CONFIG="recipes-train/qwen3.5-4b-encoder-phase0-alignment/config.yaml"

RESUME_ARG=""
INIT_PROJ_ARG=""
if [ "${1:-}" = "--resume" ]; then
    RESUME_ARG="--resume"
elif [ "${1:-}" = "--init-proj" ] && [ -n "${2:-}" ]; then
    INIT_PROJ_ARG="--init-proj ${2}"
fi

TRAIN_CMD="torchrun \
    --nproc_per_node=2 \
    --master_port=29500 \
    recipes-train/qwen3.5-4b-encoder-phase0-alignment/train.py \
    --config $CONFIG \
    $RESUME_ARG \
    $INIT_PROJ_ARG"
...
echo "Stop   : recipes-train/qwen3.5-4b-encoder-phase0-alignment/stop.sh"
