...
Answer rules:
- Move lists: SAN notation, one per line, sorted alphabetically.
...
# Sampling weights — in_check and piece_at are fast; legal_moves has longer answers
_TASK_WEIGHTS = {
    "legal_moves": 0.30,
    "captures": 0.25,
    "in_check": 0.25,
...
def main() -> None:
    ...
    parser.add_argument("--output", default="data/processed/grpo_phase1_board_reading.jsonl")
    parser.add_argument(
        "--eval-output", default="data/processed/grpo_phase1_board_reading_eval.jsonl"
    )
    ...
    task_names = list(_TASK_WEIGHTS.keys())
    task_probs = [_TASK_WEIGHTS[t] for t in task_names]
